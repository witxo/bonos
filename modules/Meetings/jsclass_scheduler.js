...
SugarWidgetListView.prototype.display = function() {
	...
	for(var i=0;i<GLOBAL_REGISTRY['result_list'].length;i++) {
		...
		html += '<td>'+bean.fields.name+'</td>';
		html += '<td>'+bean.fields.email1+'</td>';
		html += '<td>'+bean.fields.phone_office+'</td>';
		html += '<td align="right">';
		//	hidden = 'hidden';
		hidden = 'visible';
		if(!disabled) {
...
SugarWidgetSchedulerSearch.submit = function(form) {
	...
	if(form.search_first_name.value != '') {
		conditions[conditions.length] = {"name":"name","op":"starts_with","value":form.search_first_name.value}
	}
  ...
  var query = {"modules":["Accounts"
        ],"group":"and","field_list":['id','name','email1','phone_office'],"conditions":conditions};
...
SugarWidgetScheduler.fill_invitees = function(form) {
	for(var i=0;i<GLOBAL_REGISTRY.focus.users_arr.length;i++) {
		...
		} else if(GLOBAL_REGISTRY.focus.users_arr[i].module == 'Account') {
			form.account_invitees.value += GLOBAL_REGISTRY.focus.users_arr[i].fields.id + ",";          
		}
	}
}
